...
The installer picks the best available isolated installer (`uv tool` → `pipx` → `pip --user`), puts `st` and `st-mcp` on PATH, and registers `st-mcp` with Claude Code and/or Claude Desktop.
...
The MCP registration is idempotent — re-running the installer just refreshes the `command` path. Other servers in your config are preserved.
...
Then add `st-mcp` to your MCP client manually — see [MCP Server → Hooking it up to Claude Code](#mcp-server).
